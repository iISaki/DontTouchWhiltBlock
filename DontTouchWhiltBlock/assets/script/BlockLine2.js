cc.Class({
    ...
    moveDown: function() {
        ...
        let maxIndex = 0;
        for (let i = 0; i < this.Blocks.length; i++) {
            let b = this.Blocks[i].node;
            b.setPositionY(b.getPositionY() - this.speed);
            if (maxHeight < b.getPositionY()){
                maxHeight = b.getPositionY();
                maxIndex = this.Blocks[i].getLineIndex();
            }
            if (b.getPositionY() < - this.visibleSize.height / 8 * 3 && b.color.equals(cc.Color.BLACK)) {
                let blockCount =  cc.sys.localStorage.getItem("BlockCount");
                if (blockCount === undefined) {
                    blockCount = 0;
                }
                if (Number(blockCount) < this.touchNum) {
                    blockCount = this.touchNum;
                    cc.sys.localStorage.setItem("BlockCount", blockCount);
                }
                this.isStart = false;
                let alert = cc.instantiate(this.alertPrefab);
                alert.parent = this.node;
                alert.getComponent('AlertPrefab').initWithBtn(blockCount, ()=>{
                    this.restart();
                }, ()=>{
        ...
        if (maxHeight < this.visibleSize.height / 8 * 3) {
            for (let i = 0; i < this.Blocks.length; i++) {
                let b = this.Blocks[i];
                if (b.getLineIndex() < 0 ) {
                    this.Blocks.splice(i, 1);
    ...
    setTouchEvent() {
        this.node.on(cc.Node.EventType.TOUCH_START, (event)=>{
            for (let i = 0; i < this.Blocks.length; i++) {
                if (this.Blocks[i].getLineIndex() === this.canTouchIdx && cc.rectContainsPoint(this.Blocks[i].node.getBoundingBoxToWorld(), event.getLocation())) {
                    let b = this.Blocks[i];
                    if (b.node.color.equals(cc.Color.BLACK)) {
                        ...
                        //     this.speed ++;
                        // }
                        this.timeLabel.string = this.touchNum;
                    } else {
